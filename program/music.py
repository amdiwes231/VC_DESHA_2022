...
async def play(c: Client, m: Message):
    ...
    try:
        ...
        return await m.reply_text(f"خطأ يكسمك:\n\n{e}")
    a = await c.get_chat_member(chat_id, aing.id)
    if a.status != "administrator":
        await m.reply_text(
            f"💡 لاستخدامي ، أحتاج إلى أن أكون **Administrator** with the following **permissions**:\n\n» ❌ __حذف الرسائل__\n» ❌ __أضف المستخدمين__\n» ❌ __إدارة دردشة الفيديو__\n\nData is **updated** automatically after you **promote me**"
        )
        return
        ...
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
             ...
             try:
                ...
                await suhu.delete()
                await m.reply_text(f"🚫 خطأ يكسمك:\n\n» {e}")
        else:
            if len(m.command) < 2:
                await m.reply(
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                ...
                                await suhu.delete()
                                await m.reply_text(f"🚫 خطأ يكسمك: `{ep}`")
...
async def stream(c: Client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        suhu = await c.send_message(chat_id, "🔄 **جاري التحميل قليلا...**")
