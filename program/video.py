    ...
    if replied:
        if replied.video or replied.document:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo=f"{IMG_2}",
                    caption=f"💡 **تم تشغيل الفيديو يقلبي..☁️⁦❤️⁩**\n\n🏷 **Name:** [{songname}]({link})\n💭 **Chat:** `{chat_id}`\n💡 **Status:** `Playing`\n🎧 **Request by:** {requester}",
                    reply_markup=keyboard,
                )
        else:
            if len(m.command) < 2:
    ...
    if len(m.command) < 2:
        ...
        if len(m.command) == 2:
            ...
            if quality == "720" or "480" or "360":
                ...
                Q = 720
                await m.reply(
                    "» __only 720, 480, 360 allowed__ \n💡 **الآن يتدفقون الفيديو بدقة 720بكسل**"
                )
            loser = await c.send_message(chat_id, "🔄 **جاري المعالجه**")
        else:
